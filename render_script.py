...
#! 设置反射光线的最大反弹次数
bpy.context.scene.cycles.max_bounces = 4  # 总反弹次数
bpy.context.scene.cycles.glossy_bounces = 2  # 镜面反射反弹次数
#! 设置曝光值
bpy.context.scene.view_settings.exposure = -1  # 根据需要调整曝光值，通常 -1 到 -0.5 是合适的范围
#! 设置颜色管理为 Filmic
bpy.context.scene.view_settings.view_transform = 'Filmic'
bpy.context.scene.view_settings.look = 'Low Contrast'  # 可选择不同的对比度选项
#! 在 Compositing 节点中添加曝光节点
bpy.context.scene.use_nodes = True
nodes = bpy.context.scene.node_tree.nodes
exposure_node = nodes.new(type="CompositorNodeBrightContrast")
exposure_node.inputs["Bright"].default_value = -1  # 负值减少亮度
...
print('count:', len(uid_paths))

for uid_path in uid_paths[args.start_id:args.end_id]:

    # if uid_path.find('lemon') <= 0: #!!!!
    #     continue
    ...
    if ext in [".glb", ".gltf"]:
        ...
        bpy.ops.wm.usd_import(filepath=uid_path)
    else:
        raise ValueError(f'{ext} format is not supported!')

    print('begin*************')
    print('='*30, uid_path, '='*30)
    mesh_objects = [obj for obj in bpy.context.scene.objects if obj.type == 'MESH']

    if ext == '.usd':
        # 设置保存文件命名
        uid = uid_path.split('/')[-1].split('.')[0]
        category = uid_path.split('/')[-4]
        save_name = f'{category}_{uid}'
        ...
        for obj in mesh_objects:
            ...
            if len(material_names) == 1:
                ...
                continue
            link_name = material_name.replace(save_name+'_', '')
            # link_name = obj.parent.name   # 使用parent.name在玻璃材质时会无法渲染出玻璃

            # 获取材质路径
            albedo_path = os.path.join(os.path.dirname(uid_path), 'materials', f"{category}-{uid}-{link_name}-albedo.png")
            metalness_path = albedo_path.replace('albedo.png', 'metalness.png')
            normal_path = albedo_path.replace('albedo.png', 'normal.png')
            opacity_path = albedo_path.replace('albedo.png', 'opacity.png')
            roughness_path = albedo_path.replace('albedo.png', 'roughness.png')

            # 添加材质
            params = {}
            if os.path.exists(albedo_path):
                params['albedo'] = albedo_path
            else:
                print(albedo_path+' is not exists.')

            if os.path.exists(metalness_path):
                params['metalness'] = metalness_path
            else:
                print(metalness_path+' is not exists.')

            if os.path.exists(normal_path):
                params['normal'] = normal_path
            else:
                print(normal_path+' is not exists.')

            if os.path.exists(opacity_path):
                params['opacity'] = opacity_path
            else:
                print(opacity_path+' is not exists.')

            if os.path.exists(roughness_path):
                params['roughness'] = roughness_path
            else:
                print(roughness_path+' is not exists.')
            
            # 创建新的材质
            mat = bpy.data.materials.new(name=f"{link_name}_Material")
            mat.use_nodes = True
            ...
            if "albedo" in params:
                ...
                tex_image.image = bpy.data.images.load(params["albedo"])
                tex_image.location = (-300, 0)
                links.new(tex_image.outputs["Color"], bsdf.inputs["Base Color"])
            
            # Metalness 贴图
            # if "metalness" in params:
            #     metalness_image = nodes.new(type="ShaderNodeTexImage")
            #     metalness_image.image = bpy.data.images.load(params["metalness"])
            #     metalness_image.location = (-300, -150)
            #     links.new(metalness_image.outputs["Color"], bsdf.inputs["Metallic"])
            #! 设置低金属度
            ...
            if "normal" in params:
                ...
                normal_image.image = bpy.data.images.load(params["normal"])
                normal_image.location = (-300, -300)
            ...
            if "opacity" in params:
                opacity_image = nodes.new(type="ShaderNodeTexImage")
                opacity_image.image = bpy.data.images.load(params["opacity"])
                opacity_image.location = (-300, -450)
                links.new(opacity_image.outputs["Color"], bsdf.inputs["Alpha"])
                mat.blend_method = 'BLEND'  # 启用透明混合模式
            
            # Roughness 贴图
            # if "roughness" in params:
            #     roughness_image = nodes.new(type="ShaderNodeTexImage")
            #     roughness_image.image = bpy.data.images.load(params["roughness"])
            #     roughness_image.location = (-300, -600)
            #     links.new(roughness_image.outputs["Color"], bsdf.inputs["Roughness"])
            #! 设置高粗糙度
            bsdf.inputs["Roughness"].default_value = 0.8

            if obj.data.materials:
                obj.data.materials[0] = mat  # 替换现有材质
            else:
                obj.data.materials.append(mat)  # 添加新的材质
    ...
    for camera_opt in range(-1, 8):
        ...
        if camera_opt == -1:
            ...
            img_path = os.path.join(args.parent_dir, 'Cap3D_imgs', 'Cap3D_imgs_view_bg', '%s_bg.png'%(save_name))
        ...
        if camera_opt == -1:
            file_path = os.path.join(args.parent_dir, 'Cap3D_imgs', 'Cap3D_imgs_view_bg', '%s_bg.png'%(save_name))
            bpy.context.scene.render.filepath = file_path
            if os.path.exists(file_path):
               continue
        else:
            file_path = os.path.join(args.parent_dir, 'Cap3D_imgs', 'Cap3D_imgs_view%d'%camera_opt, '%s_%d.png'%(save_name, camera_opt))
            bpy.context.scene.render.filepath = file_path
            if os.path.exists(file_path):
               continue
        ...
        if camera_opt>=0:
            ...
            RT_path = os.path.join(args.parent_dir, 'Cap3D_imgs', 'Cap3D_imgs_view%d_CamMatrix'%camera_opt, '%s_%d.npy'%(save_name, camera_opt))
