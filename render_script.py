...
for uid_path in uid_paths:
    ...
    if ext == '.usd':
        #!!! 设置材质
        obj = bpy.data.objects['visuals']
        # 2. 创建新的材质
        material = bpy.data.materials.new(name="TextureMaterial")
        material.use_nodes = True  # 启用节点
        ...
        # 在此加载图像文件 (例如 "C:/path/to/your_image.jpg")
        image_path = None
        material_path = os.path.dirname(uid_path) + '/materials'
        files = os.listdir(material_path)
        for f in files:
            if f.endswith('albedo.png'):
                image_path = os.path.join(material_path, f)
                break
        ...
        links.new(tex_image.outputs["Color"], bsdf.inputs["Base Color"])  # 将图像输出连接到 BSDF 基础颜色
        links.new(bsdf.outputs["BSDF"], material_output.inputs["Surface"])  # 将 BSDF 输出连接到材质输出

        # 6. 将材质分配给物体
        if obj.data.materials:
            obj.data.materials[0] = material  # 替换现有材质
        else:
            obj.data.materials.append(material)  # 添加新的材质
